...
const Features = () => {
  ...
  <div className="grid grid-cols-3 max-sm:flex max-xl:flex-col min-h-screen w-full grid-rows-3 gap-5 mt-5 sm-max:p-2">
    <div className="rounded row-span-3 w-full h-full max-sm:h-fit bg-gray-950">
      <Canvas>
        <ambientLight />
    ...
    <div className="rounded col-span-2 w-full h-full bg-gray-800 row-span-2 py-10 px-10 flex flex-col gap-2">
      <div className="text-xl max-sm:text-lg font-semibold text-gray-300">
        Meet Alex! He might look a little sad at first, but greet him, and he’ll quickly become your best buddy.
      </div>
      <button onClick={handleWaveAnimation} className="place-self-end text-lg max-sm:text-base font-bold bg-blue-600 hover:bg-blue-700 transition-colors rounded p-1 px-3 mb-2 text-white">
        Say Hi to Alex!
      </button>
      <div className="text-lg max-sm:text-base text-gray-400">
      ...
      <div className="text-lg max-sm:text-base text-gray-400 mt-4">
        <h3 className="text-lg max-sm:text-base font-semibold text-gray-300">Features of Rapid Shell:</h3>
      ...
      <div className="flex max-sm:flex-col">
        <button onClick={handleChickenDanceAnimation} className="outline outline-white outline-offset-2 mt-4 w-fit max-sm:w-auto text-lg max-sm:text-base font-bold bg-green-600 hover:bg-green-700 transition-colors rounded p-2 text-white">
          {buttonText} {/* Display the dynamic button text */}
        </button>
        {playHappy && ( // Only render this div when playHappy is true
          <div className="text-xl max-sm:text-lg mt-4 w-fit p-2 px-10 font-semibold text-gray-700">
